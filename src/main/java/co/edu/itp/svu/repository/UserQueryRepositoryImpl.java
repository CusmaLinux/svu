...
public class UserQueryRepositoryImpl implements UserQueryRepository {
    ...
    @Override
    public Page<User> search(String queryString, Pageable pageable) {
        Query query = new Query().with(pageable);

        if (StringUtils.hasText(queryString)) {
            List<Criteria> orExpressionCriteria = new ArrayList<>();
            orExpressionCriteria.add(Criteria.where("login").regex(queryString, "i"));
            orExpressionCriteria.add(Criteria.where("email").regex(queryString, "i"));
